...
begin
  ...
  // pessoa fisica
    fxcontrato.LoadFromFile('\SQL_Gerencial\server\rel\contrato_reserva.fr3')
  else
  begin
    // pessoa juridica
    if qrcliente.FieldByName('resp2_nome').asstring <> '' then
    // dois responsaveis
      fxcontrato.LoadFromFile('\SQL_Gerencial\server\rel\contrato_reserva_j2.fr3')
    else
    // um responsavel
      fxcontrato.LoadFromFile('\SQL_Gerencial\server\rel\contrato_reserva_j1.fr3')
...
begin
  arquivo_ini := TIniFile.Create('C:\SQL_Gerencial\server\dll\contrato.ini');
