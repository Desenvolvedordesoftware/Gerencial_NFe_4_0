...
type
  Tfrmos_item_servico = class(TForm)
...
var
  frmos_item_servico: Tfrmos_item_servico;
  VER_PRODUTO: BOOLEAN;
...
procedure Tfrmos_item_servico.bcancelarClick(Sender: TObject);
begin
  lserial.caption := '';
  lfornecedor.Caption := '';
  rqtde.value := 0;
...
procedure Tfrmos_item_servico.rqtdeEnter(Sender: TObject);
begin
  tedit(sender).color := $00A8FFFF;
end;

procedure Tfrmos_item_servico.rqtdeExit(Sender: TObject);
begin
...
procedure Tfrmos_item_servico.runitarioEnter(Sender: TObject);
begin
...
procedure Tfrmos_item_servico.rdesconto1Enter(Sender: TObject);
begin
...
procedure Tfrmos_item_servico.rdesconto2Enter(Sender: TObject);
begin
...
procedure Tfrmos_item_servico.runitarioExit(Sender: TObject);
begin
  tedit(sender).color := clwindow;
...
procedure Tfrmos_item_servico.rdesconto1Exit(Sender: TObject);
begin
...
procedure Tfrmos_item_servico.rdesconto2Exit(Sender: TObject);
begin
  tedit(sender).color := clwindow;
...
procedure Tfrmos_item_servico.rqtdeKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
  begin
...
procedure Tfrmos_item_servico.rdesconto2KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then bgravar.setfocus;
end;

procedure Tfrmos_item_servico.FormCreate(Sender: TObject);
begin
...
procedure Tfrmos_item_servico.eprodutoEnter(Sender: TObject);
begin
  TEDIT(SENDER).COLOR := $00A8FFFF;
end;

procedure Tfrmos_item_servico.eprodutoExit(Sender: TObject);
begin
  TEDIT(SENDER).COLOR := CLWINDOW;
end;

procedure Tfrmos_item_servico.EPRODUTOKeyPress(Sender: TObject; var Key: Char);
...
begin
  ...
  begin
    ...
    begin
      ...
      qrproduto.Close;
      qrproduto.sql.clear;
      qrproduto.SQL.Add('select * from c000011 where codigo =''' + CODIGO + '''');
      qrproduto.Open;
      ...
      begin
        ...
        qrproduto.Close;
        qrproduto.sql.clear;
        qrproduto.SQL.Add('select * from c000011 where servico =''' + eproduto.text + '''');
        qrproduto.Open;
      ...
      begin
        ...
        begin
          ...
          qrproduto.Close;
          qrproduto.sql.clear;
          qrproduto.SQL.Add('select * from c000011 where codigo =''' + a + '''');
          qrproduto.Open;
     ...
     // procurar pelo codigo de barras da grade
        QUERY.Close;
        QUERY.sql.clear;
        QUERY.SQL.Add('select * from c000011 where servico =''' + eproduto.text + '''');
        QUERY.Open;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            qrproduto.Close;
            qrproduto.sql.clear;
            qrproduto.SQL.Add('select * from c000011 where servico like ''%' + eproduto.text + '%''');
            qrproduto.Open;

            if qrproduto.RecordCount = 0 then
            begin
              application.messagebox('SEVIÇO não encontrado!', 'Aviso', mb_ok + MB_ICONWARNING);
              exit;
            end
            else
            begin
...
//                  frmloc_venda_produto_auto.ShowModal;
                  ...
                  begin
                    qrproduto.Close;
                    qrproduto.sql.clear;
                    qrproduto.SQL.Add('select * from c000011 where codigo like ''%' + produto_loc + '%''');
                    qrproduto.Open;
                    if qrproduto.RecordCount > 0 then achou := true else achou := false;
                  end;
                end
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            frmos.qrproduto_serial.sql.Add('select * from c000011 where codigo = ''' + COD_serial + ''' and situacao = 1 ');
          ...
          begin
            frmos.qrproduto_serial.close;
            frmos.qrproduto_serial.sql.clear;
            frmos.qrproduto_serial.sql.Add('select * from c000011 where codigo = ''' + qrproduto.fieldbyname('codigo').asstring + ''' and situacao = 1 order by codigo');
            frmos.qrproduto_serial.open;
      ...
      begin
        application.messagebox('SERVIÇO não encontrado!', 'Aviso', mb_ok + MB_ICONWARNING);
        eproduto.text := '';
        eproduto.SetFocus;
      end;
    end
...
procedure Tfrmos_item_servico.ecodigoExit(Sender: TObject);
begin
  TEDIT(SENDER).COLOR := CLWINDOW;
end;

procedure Tfrmos_item_servico.bgravarClick(Sender: TObject);
var aliquota: string;
  unidade: string;
begin
  if eproduto.text <> '' then
  begin
    ...
    frmos.qros_produtos.FieldByName('codigo').asstring := qrproduto.fieldbyname('codigo').asstring;
    ...
    frmos.qros_produtos.fieldbyname('servico').asstring := eproduto.text;
  ...
  begin
    application.messagebox('Favor informar o SERVIÇO!', 'Atenção!', mb_ok + MB_ICONWARNING);
    eproduto.setfocus;
  end;
...
procedure Tfrmos_item_servico.FormShow(Sender: TObject);
begin
...
procedure Tfrmos_item_servico.ecstKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl, 0, 0);
end;

procedure Tfrmos_item_servico.ealiquotaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl, 0, 0);
end;
