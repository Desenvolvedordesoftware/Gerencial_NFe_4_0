...
type
  Tfrmos = class(TForm)
    ...
    qros_servicoVLR_TOTAL: TIntegerField;
    cadastro_cliente_os: TAdvSmoothButton;
    Panel8: TPanel;
    ...
    procedure bincluir_servicoClick(Sender: TObject);
    procedure qros_servicoCalcFields(DataSet: TDataSet);
    procedure cadastro_cliente_osClick(Sender: TObject);
...
procedure Tfrmos.cadastro_cliente_osClick(Sender: TObject);
begin
...
procedure Tfrmos.bincluir_servicoClick(Sender: TObject);
{begin
var item, qtde, total: real;}
begin
  //frmos_item_servico := tfrmos_item_servico.create(self);
 // frmos_item_servico.showmodal;
...
procedure Tfrmos.qros_servicoCalcFields(DataSet: TDataSet);
begin
qros_servicoVLR_TOTAL.AsInteger:=qros_servicoVALOR.AsInteger * qros_servicoQTDE.AsInteger;
end;
...
begin
  valor_anterior := qros_servico.fieldbyname('VLR_TOTAL').asfloat;
end;
...
begin
  frmmodulo.cdsos.fieldbyname('servico_subtotal').asfloat :=
    frmmodulo.cdsos.fieldbyname('servico_subtotal').asfloat +
    qros_servico.fieldbyname('VLR_TOTAL').asfloat -
    valor_anterior;
...
begin
  ...
  begin
    ...
    frmOS_contasreceber.rsoma.value :=
      frmOS_contasreceber.rsoma.value +
      qrOS_contasreceber.fieldbyname('VLR_TOTAL').asfloat -
      valor_prestacao;
...
begin
  valor_prestacao := qrOS_contasreceber.fieldbyname('VLR_TOTAL').asfloat;
end;
...
procedure Tfrmos.bexcluirClick(Sender: TObject);
var
...
//    exit;
          ...
          if application.messagebox
            (pwidechar('Cancelar O.S?' + #13 ), 'Aviso', mb_yesno + MB_ICONWARNING) = idno then
            exit;
  ...
  qrcan_os.Close;
  qrcan_os.sql.clear;
  qrcan_os.sql.add('delete from c000051 ');
  qrcan_os.sql.add('where ' + FRMMODULO.cdsos.fieldbyname('codigo')
      .asstring);


  Application.MessageBox('O.S cancelada com sucesso!', 'aviso', mb_ok + MB_ICONWARNING);
  CLOSE;
  //    exit;
end;
