...
uses modulo, principal, loc_regiao, regiao, loc_funci,
  ...
  cliente_inicio, xloc_cidade, webcam2, xloc_cnae, loc_paises;
...
begin
  if (combo_pessoa.Text = 'JURIDICA') then begin
...
begin
  ...
  if (FRMMODULO.qrcliente.State = dsinsert) or
    (FRMMODULO.qrcliente.State = dsedit) then
  begin
    FRMMODULO.qrcliente.FieldByName('cpais').asstring :=
    frmprincipal.zerarcodigo(RzDBButtonEdit1.Text, 4);
...
begin
  ...
  begin
    ...
    begin
      ...
      FRMMODULO.qrclienteIBGE.asstring := ACBrConsultaCNPJ1.IBGE_UF;
      ...
      qrCid.Close;
      qrCid.SQL.Text := 'select * from c000006  cid where cid.CIDADE = ' + QuotedStr(ACBrConsultaCNPJ1.Cidade) + ' and UF = ' + QuotedStr(ACBrConsultaCNPJ1.UF);
      qrCid.Open;
      if qrCid.RecordCount = 1 then begin
        ...
        parametro_pesquisa := ACBrConsultaCNPJ1.Cidade;
        frmxloc_cidade := tfrmxloc_cidade.CREATE(SELF);
        frmxloc_cidade.SHOWMODAL;
        FRMMODULO.qrcliente.FieldByName('COD_MUNICIPIO_IBGE').asstring := resultado_pesquisa6;
        FRMMODULO.qrcliente.FieldByName('IBGE').asstring := resultado_pesquisa5;
