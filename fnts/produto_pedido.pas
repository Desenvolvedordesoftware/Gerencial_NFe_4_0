...
begin
  ...
  if not DirectoryExists('C:\SQL_Gerencial\Server\PDF') then
  begin
    ForceDirectories('C:\SQL_Gerencial\server\PDF');
  end;
  ...
  PDF.DefaultPath := 'C:\SQL_Gerencial\Server\PDF\';
  PDF.ShowDialog := false;
  PDF.ShowProgress := false;
  PDF.OverwritePrompt := false;
  fxpedido.LoadFromFile('\SQL_Gerencial\server\rel\f000132_2.fr3');
  ...
  TIdAttachmentFile.create(EmailCorpo.MessageParts,
    Tfilename('C:\SQL_Gerencial\Server\PDF\' + DBEdit2.Text + '-' + DBEdit23.Text
    + '.pdf'));
...
begin
  ...
  fxpedido.LoadFromFile('\SQL_Gerencial\server\rel\f000132.fr3');
  fxpedido.ShowReport;
end;
