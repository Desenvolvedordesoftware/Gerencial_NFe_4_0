...
uses modulo, principal, loc_cliente, notas_venda_impressao,venda_impressao,
  ...
  if frmmodulo.cancelou_venda_nf then begin
    ...
    while frmmodulo.qrproduto_mov.recordcount <> 0 do begin
      if frmmodulo.qrproduto.Locate('codigo',
        frmmodulo.qrproduto_mov.FIELDBYNAME('codproduto').AsString,[loCaseInsensitive]) then begin
        // DEVOLVER LOTE
      ...
      if APPLICATION.MESSAGEBOX('Esta venda possui Nota fiscal!' + #13 + 'Deseja abrir a tela de Notas Fiscais para seu cancelamento?', 'Aviso', MB_YESNO + mb_iconwarning) = idYes then begin
        ...
        if frmmodulo.cancelou_venda_nf then begin
          ...
          QUERY.sql.add('delete from c000124 where cod_venda = ''' +
            frmmodulo.QRVENDA.FIELDBYNAME('CODIGO')
            .AsString + '''');
          QUERY.ExecSQL;
...
procedure Tfrmnotas_venda.AdvGlowButton3Click(Sender: TObject);
 begin
 QUERY.close;
    QUERY.sql.clear;
    QUERY.sql.add('select * from c000048 where codigo = ''' + frmmodulo.QRVENDA.FIELDBYNAME('codigo').AsString + '''');
    QUERY.Open;
    // ecfs tratar depois
