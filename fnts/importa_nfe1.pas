...
uses
  ...
  JvExStdCtrls, JvEdit, JvValidateEdit, cxGraphics, cxLookAndFeels,System.Math,
  cxLookAndFeelPainters, cxButtons, JvExMask, JvToolEdit, JvBaseEdits,
...
uses ACBrNFeNotasFiscais, pcnNFe, MaskUtils, loc_conta, modulo, principal,
  Produto, UFuncoes, status, chavenfe_importar, xloc_produto;
    ...
    while not qritem.eof do begin
      ...
      if qritemCODPRODUTO.AsString <> '' then begin
        ...
        if query.RecordCount > 0 then
          ...
          CadastrarProduto(qritemCODLANCAMENTO.AsString,
            ...
            qritemCODFORNECEDOR.AsString, qritemUN_COMPRA.AsString,
            qritemCLASSIFICACAO_FISCAL.AsString, qritemCSOSN.AsString, qritemCST.AsString,
      ...
      begin
        CadastrarProduto(qritemCODLANCAMENTO.AsString, qritemCODBARRA.AsString,
          ...
          qritemUN_FRACAO.AsString, qritemCLASSIFICACAO_FISCAL.AsString,
          qritemCSOSN.AsString, qritemCST.AsString, qritemIPI_ALIQUOTA.ASFLOAT,
...
{$ENDIF}
    ...
    ACBrNFe1.Configuracoes.Geral.FormaEmissao := StrToTpEmis(ok, IntToStr(Ini.ReadInteger('Geral', 'FormaEmissao', 0) + 1));
    ...
    ACBrNFe1.Configuracoes.WebServices.UF := Ini.ReadString('WebService','UF', 'AM');
    ACBrNFe1.Configuracoes.WebServices.Ambiente := StrToTpAmb(ok, IntToStr(Ini.ReadInteger('WebService', 'Ambiente', 0) + 1));
...
begin
  ...
  begin
    ...
    end else if Pos(qritemCFOP_CAD.AsString,'5405, 5403, 5408, 5401, 6405, 6403, 6408, 6401') > 0 then begin
...
var
  ...
  vcodmunicipio, vdv,vemitente, vdestinatario, vemitente_cnpj,
  vdestinatario_cnpj, vcod_item, vcod_fornecedor, vcod_produto,vcidade,
  vcep, vcomplemento, vtelefone1, vtelefone2, vfax, vcontato1,vcontato2,
  vcelular1, vcelular2, vemail, vhomepage, vcnpj, vie, vbanco, vagencia,
  vconta, vobs1, vobs2, vobs3, vnumero, vim, vcod_municipio_ibge, vibge,
  vnomeprod, vxBairro, vxLgr, vfantasia, vmunemitente, vmundestinatario,
  vdata, idLote, TipoFator, lMsg, scfpotmp: string;
  ...
  try
    ...
    for x := 0 to eitems.AsInteger - 1 do begin
      ...
      if vcod_produto <> '' then begin
        ...
        qrConversao.SQL.Add('where uni.codproduto = ' + QuotedStr(Trim(vcod_produto)) + ' and upper(uni.un_origem) = ' +
        UpperCase(QuotedStr(Trim(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.uTrib))) + ' and upper(uni.un_destino) = ' +
        UpperCase(QuotedStr(Trim(query.FieldByName('UNIDADE').AsString))));
        qrConversao.Open;
      ...
      if (Length(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.cEAN) > 0) AND (
          Length(vcodBarra_produto) = 0) then
      ...
      end else if (scfpotmp = '54') or (scfpotmp = '64') then begin
...
{      if ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Imposto.ICMS.cst = cstVazio then
      ...
      if (ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.CFOP = '5405') or
        ...
        qritemCFOP.AsString := '2403'
              ...
              (ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.CFOP = '6911')then
        qritemCFOP.AsString := '2102'
      else if (ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.CFOP = '5910') or
              (ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.CFOP = '5911') then
        qritemCFOP.AsString := '1102'
      ...
      { : PREÇOS }
      qritemPRECOCUSTO.ASFLOAT :=RoundTo((qritemTOTAL.ASFLOAT + qritemFRETE.ASFLOAT +
        qritemSEGURO.ASFLOAT + qritemOUTRAS.ASFLOAT + qritemIPI_VALOR.ASFLOAT) /
        (qritemQTDE.ASFLOAT) / qritemFRACAO.ASFLOAT,-2);
      ...
      if vPrecoVenda > 0 then
        ...
        qritemPRECOVENDA.ASFLOAT := RoundTo(qritemPRECOCUSTO.ASFLOAT /
          qritemFRACAO.ASFLOAT,-2);

      qritemPMARGEM.Value := RoundTo((((qritemPRECOVENDA.ASFLOAT - qritemPRECOCUSTO.ASFLOAT) /
        qritemPRECOCUSTO.ASFLOAT)) * 100,-2);
      { : PREÇOS }
...
begin
  ...
  if edAtual.Checked then begin
    qritem.Edit;
    qritemPRECOVENDA.AsFloat := qritemPRECOVENDA.AsFloat + ((qritemPRECOVENDA.AsFloat * edLucro.Value)/100);
    qritem.Post;
    qritem.ApplyUpdates;
  end else begin
    qritem.First;
...
begin
  ...
  fximprime.LoadFromFile('\SQL_Gerencial\server\REL\F000197.fr3');
